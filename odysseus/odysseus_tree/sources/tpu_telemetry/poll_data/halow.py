...
def fetch_data_ApMCS():
    try:
        out = check_output(FETCH_RATE_CMD.split(" "), shell=True)
...
def fetch_data_StaMCS():
    try:
        out = check_output(FETCH_RATE_CMD.split(" "), shell=True)
        data_line = example_data.splitlines()[2] 
...
def fetch_data_RSSI():
    ...
    try:
        out = check_output(FETCH_RSSI_CMD.split(" "), shell=True)
        data = out.splitlines()[1].split()[2]
