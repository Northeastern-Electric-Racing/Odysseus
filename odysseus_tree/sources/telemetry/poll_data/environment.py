...
class EnvironmentMT(MeasureTask):
    ...
    def measurement(self):
        ...
        if float(temperature) is not None:
            data.append(
                (f"{self.topic_root}/Environment/Temperature", [temperature], "Celsius")
            )

        humidity = self.read_sensor_data(
            HUMIDITY_SENSOR_PATH="/sys/class/hwmon/hwmon2/humidity1_input"
        )
        if float(humidity) is not None:
            data.append((f"{self.topic_root}/Environment/Humidity", [humidity], "%"))

        return data


def main():
    ex = EnvironmentMT("TPU")
